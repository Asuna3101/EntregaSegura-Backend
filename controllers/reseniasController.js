...
const getReseniasByUsuarioId = (req, res) => {
  ...
  if (!usuario_id) {
    return res.status(400).json({
      error: "El ID del usuario es necesario para realizar la consulta.",
    });
  }
...
const createResenia = (req, res) => {
  const { usuario_id, repartidor_id, estrellas, resenia } = req.body;

  console.log(req.body);

  if (!usuario_id || !repartidor_id || estrellas == null || !resenia) {
    return res
      .status(400)
  ...
  db.run(
    query,
    [usuario_id, repartidor_id, estrellas, resenia],
    function (err) {
      if (err) {
        console.error("Error al insertar la calificación:", err.message);
        return res
          .status(500)
          .json({ ok: false, error: "Error al crear la calificación." });
      }
