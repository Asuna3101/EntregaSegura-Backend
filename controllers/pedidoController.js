...
const deletePedido = (req, res) => {
    ...
    if (!pedido_id) {
        return res.status(400).json({ error: 'El ID del pedido es necesario para realizar la operación.' });
    }
...
const getSeguimientoPedido = (req, res) => {
    ...
    if (!pedido_id) {
        return res.status(400).json({ error: 'El ID del pedido es necesario para realizar la consulta.' });
    }
    ...
    db.get(query, [pedido_id], (err, row) => {
        if (err) {
            console.error('Error al consultar el seguimiento del pedido:', err.message);
            return res.status(500).json({ error: 'Error al recuperar el seguimiento del pedido.' });
        }
        if (row) {
